...
// =========================================================
// 🟢 CONFIGURATION ROBOT 2026 (FINAL v15 - FLYING MODE)
// =========================================================
const config = window.robotConfig || { mode: 'attente', titre: 'Événement', logo: '' };

const DUREE_LECTURE = 6000; // Un peu plus court pour être dynamique
const ECHELLE_BOT = 0.6; 
const SAFE_ZONE_X = 10.0; 
const MUTE_LIMIT_X = 9.5; // Si X > 9.5, il se tait (avant de toucher le bord)

// Vitesses
const SPEED_NORMAL = 0.008;
const SPEED_IMPULSE = 0.04; // Très vite pour les traversées
...
// --- SCENARIOS (Adaptés pour Vol) ---
const introScript_Attente = [
    { time: 2, text: "", action: "teleport_in_left" }, 
    { time: 5, text: "Wouah... C'est grand ici !", type: "thought", action: "fly_right_up" }, 
    { time: 12, text: "Je crois que je suis le premier...", type: "thought", action: "fly_left_down" }, 
    { time: 20, text: "OH ! Il y a du monde ! 😳", type: "speech", action: "stop_left" }, 
    { time: 27, text: "Bonjour ! Je suis Clap-E ! 👋", type: "speech", action: "wave_hands" },
    { time: 35, text: "Bienvenue à tous !", type: "speech", action: "fly_right_center" }, 
    { time: 42, text: "", action: "impulse_cross_left" }, // TRAVERSEE RAPIDE SILENCIEUSE
    { time: 45, text: "Ouhlà ! Ça décoiffe !", type: "speech", action: "stop_right" },
    ...
    { time: 122, text: "", action: "impulse_exit_right" }, // SORTIE RAPIDE
    { time: 135, text: "Me revoilà ! 😅", type: "speech", action: "teleport_in_left" }, 
    { time: 142, text: "C'était moins une !", type: "speech", action: "fly_center_hover" },
    { time: 150, text: "Installez-vous bien, je veille.", type: "speech" }
];

const introScript_VoteOff = [
    { time: 2, text: "", action: "teleport_in_right" }, 
    ...
    { time: 27, text: "", action: "impulse_cross_right" }, // TRAVERSEE
    { time: 30, text: "Soyez patients, ça arrive !", type: "speech", action: "stop_left" }, 
    { time: 37, text: "C'était serré... 🤫", type: "speech" }
];
...
const introScript_Photos = [
    ...
    { time: 33, text: "", action: "impulse_cross_left" }, // ZOOM
    { time: 36, text: "J'ai failli percuter une bulle !", type: "speech", action: "stop_right" }, 
    { time: 44, text: "Un petit selfie avec Clap-E ?", type: "speech", action: "pose_selfie" }, 
    { time: 50, text: "Bon, je file voir la régie...", type: "speech" }, 
    { time: 56, text: "", action: "impulse_exit_left" }, // SORTIE
    { time: 65, text: "Me revoilà ! ✨", type: "speech", action: "teleport_in_right" }, 
    { time: 72, text: "Allez, tous ensemble !", type: "speech", action: "pose_selfie" } 
];
...
function initThreeJS(canvasFloor, canvasBot, bubbleEl) {
    ...
    function triggerTeleport(pos) {
        ...
        particles.position.copy(pos); teleportLight.position.copy(pos); teleportLight.intensity = 5;
    ...
    let isWaving = false;
    let isImpulsing = false; // NOUVEAU FLAG : Est en traversée rapide ?
    ...
    function showBubble(text, type = 'speech') { 
        // SI IMPULSION OU HORS ECRAN = PAS DE TEXTE
    ...
    function pickNewTarget() {
        // LOGIQUE "VOLANT" (Hover ou Impulsion)
        const r = Math.random();
        
        if (r < 0.3) { // 30% Chance : IMPULSION (Traversée)
            isImpulsing = true;
            // Choisir un coté de départ (Hors écran)
        ...
        } else { // 70% Chance : VOL STATIONNAIRE (Sur les cotés)
            isImpulsing = false;
            const side = Math.random() > 0.5 ? 1 : -1;
            // Zones latérales safe (X entre 9 et 13)
            const randomX = side * (SAFE_ZONE_X + Math.random() * 3);
    ...
    function animate() {
        ...
        if (robotState === 'intro') {
            ...
            if (step && time >= step.time) {
                ...
                // GESTION SPECIALE : Si impulsion, pas de texte
                let allowText = true;
                ...
                // Mouvements standards "Volants"
                if(step.action === "fly_right_up") targetPos.set(11, 4, -2);
                if(step.action === "fly_left_down") targetPos.set(-11, -2, 2);
                if(step.action === "fly_right_center") targetPos.set(12, 0, 0);
                if(step.action === "fly_left_check") targetPos.set(-12, -1, 0);
                if(step.action === "fly_scan") targetPos.set(-10, 3, 3);
                if(step.action === "fly_loop") targetPos.set(10, 2, -3);
                if(step.action === "fly_center_hover") targetPos.set(0, 3, 0); // Exception : Hover haut centre

                // ACTIONS SPECIFIQUES
                if(step.action === "stop_left") targetPos.set(-11, 0, 4);
                if(step.action === "stop_right") targetPos.set(11, 0, 4);
            ...
            if(robotState !== 'dancing_intro' && robotState !== 'posing') {
                // Vitesse dynamique : Tres vite si Impulsion, normal sinon
                const spd = isImpulsing ? SPEED_IMPULSE : VITESSE_MOUVEMENT;
                robotGroup.position.lerp(targetPos, spd);
            }
        } 
        
        // --- BOUCLE INFINIE (Volant & Impulsions) ---
            ...
            // Mouvement : Si Impulsion = Vitesse Max, sinon Vol tranquille
            const spd = isImpulsing ? SPEED_IMPULSE : VITESSE_MOUVEMENT;
            robotGroup.position.lerp(targetPos, spd);
            
            // Animation de vol (Tangage)
            robotGroup.rotation.z = (targetPos.x - robotGroup.position.x) * -0.05; // Penche dans le virage
            robotGroup.rotation.x = (targetPos.y - robotGroup.position.y) * 0.05; 
            
            // Arrivée à destination
            if(robotGroup.position.distanceTo(targetPos) < 1.0) {
                if(isImpulsing) { isImpulsing = false; } // Fin d'impulsion
                pickNewTarget();
            }
            
            if(time > nextEvt && !isImpulsing) { // Pas d'event si en impulsion
                const r = Math.random();
        ...
        // GESTION BULLE ET COUPURE BORDS
        if(bubbleEl && bubbleEl.style.opacity == 1) {
            // COUPURE STRICTE
            if (isImpulsing || Math.abs(robotGroup.position.x) > MUTE_LIMIT_X) {
                bubbleEl.style.opacity = 0;
            } else {
                const headPos = robotGroup.position.clone(); headPos.y += 1.3; headPos.project(cameraBot);
