...
// =========================================================
// 🟢 CONFIGURATION ROBOT 2026 (FLUIDE & ALWAYS ON SCREEN)
// =========================================================
...
const Y_MAX = 3.5;
const Z_NORMAL = 0;
const Z_CLOSEUP = 7.5; // Très proche de l'écran
...
style.innerHTML = `
    .robot-bubble-base {
        position: fixed; padding: 15px 25px; color: black; font-family: 'Arial', sans-serif;
...
function initThreeJS(canvasFloor, canvasBot, bubbleEl) {
    ...
    // CONSTRUCTION ROBOT & PARTS (CORRECTION BUG PARTS)
    const robotGroup = new THREE.Group(); 
    robotGroup.position.set(0, 0, 0); // Départ Centre
    robotGroup.scale.set(ECHELLE_BOT, ECHELLE_BOT, ECHELLE_BOT);
    
    const parts = []; // Tableau vital pour l'explosion
    ...
    // LOGIQUE MOTEUR
    let time = 0;
    let targetPos = new THREE.Vector3(0, 0, Z_NORMAL);
    let state = 'idle'; // idle, exploding, reassembling, closeup, teleporting
    let nextEventTime = 0;
    let isWaving = false;
    let textMsgIndex = 0;
    ...
    function decideNextAction() {
        // Reste toujours sur l'écran : pick random pos within bounds
        const x = (Math.random() * (X_MAX - X_MIN)) + X_MIN;
        const y = (Math.random() * (Y_MAX - Y_MIN)) + Y_MIN;
        targetPos.set(x, y, Z_NORMAL);
        ...
        if (r < 0.15) {
            state = 'closeup';
            targetPos.set(0, 0, Z_CLOSEUP); // Vient au centre très près
            showBubble("Je vous vois de près !", 'thought');
            setTimeout(() => { targetPos.z = Z_NORMAL; state = 'idle'; }, 4000);
        }
        // 2. EXPLOSION (10%)
        else if (r < 0.25) {
            state = 'exploding';
            showBubble("Oups ! Surchauffe !", 'thought');
            // Init explosion velocities
            parts.forEach(p => {
                p.userData.velocity.set((Math.random()-0.5)*0.5, (Math.random()-0.5)*0.5, (Math.random()-0.5)*0.5);
            });
        ...
        // 3. TELEPORTATION (15%)
            ...
            setTimeout(() => {
                // Change de coté instantanément (X devient -X ou random)
                const newX = (Math.random() > 0.5) ? -7 : 7;
                robotGroup.position.set(newX, 0, 0);
                targetPos.set(newX, 0, 0);
                triggerTeleportEffect(robotGroup.position);
                showBubble("Téléportation !", 'speech');
                state = 'idle';
            }, 500);
        }
        // 4. MOUVEMENT STANDARD + PAROLE (60%)
        else {
            state = 'idle';
            const msg = getNextMessage();
            showBubble(msg, 'speech');
            isWaving = Math.random() > 0.7; // Parfois il fait coucou
            if(isWaving) setTimeout(() => { isWaving = false; }, 3000);
        }

        // Prochaine décision dans 6 à 10 secondes
    ...
    function animate() {
        ...
        // LOGIQUE D'ETATS
        if (state === 'idle' || state === 'closeup') {
            // Mouvement Fluide (Lerp) vers targetPos
            robotGroup.position.lerp(targetPos, 0.02);
            
            // Floating effect (respiration)
            robotGroup.position.y += Math.sin(time * 2) * 0.005;
            robotGroup.rotation.z = Math.cos(time) * 0.05; // Léger tangage
            robotGroup.rotation.y = Math.sin(time * 0.5) * 0.1; // Léger regard gauche/droite
            ...
            parts.forEach(p => {
                ...
                p.userData.velocity.multiplyScalar(0.95); // Ralentissement
            });
        }
            ...
            parts.forEach(p => {
                ...
                // Slerp manuel pour rotation (simple lerp ici suffit pour visuel rapide)
        ...
        // ANIMATION BRAS (WAVING)
        if (isWaving && state === 'idle') {
            armLGroup.rotation.z = Math.sin(time * 15) * 0.5;
