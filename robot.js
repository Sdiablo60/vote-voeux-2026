...
// =========================================================
// 🟢 CONFIGURATION ROBOT 2026 (FINAL v14 - FX BOOST & TIMING)
// =========================================================
...
// Limite plus stricte : dès qu'il touche le bord, le texte coupe
...
style.innerHTML = `
    .robot-bubble-base {
        ...
        pointer-events: none; transition: opacity 0.2s, transform 0.3s; transform: scale(0.9); 
...
// --- SCENARIO 1 : ACCUEIL (CORRIGÉ : TELEPORT AU DEBUT + FX) ---
const introScript_Attente = [
    // APPARITION IMMEDIATE (Pas de marche depuis l'extérieur)
    { time: 2, text: "", action: "enter_teleport_left_visible" }, 
    { time: 5, text: "Wouah... C'est grand ici !", type: "thought", action: "look_around" }, 
    { time: 13, text: "Je crois que je suis le premier arrivé...", type: "thought", action: "move_right_slow" }, 
    { time: 21, text: "Tiens ? C'est quoi cette lumière ?", type: "thought", action: "move_left_check" }, 
    { time: 29, text: "OH ! Mais... Il y a du monde en fait ! 😳", type: "speech", action: "surprise_stop" }, 
    
    { time: 37, text: "Bonjour tout le monde ! Je suis Clap-E ! 👋", type: "speech", action: "wave_hands" },
    
    { time: 45, text: "Vous êtes nombreux ce soir ! Bienvenue !", type: "speech", action: "move_far_right" }, 
    { time: 53, text: "", action: "toc_toc_approach" }, 
    { time: 55, text: "Toc ! Toc ! Vous m'entendez là-dedans ?", type: "speech" }, 
    { time: 63, text: "Ah ! Vous êtes bien réels ! 😅", type: "speech", action: "backup_a_bit" }, 
    { time: 71, text: "Votre visage me dit quelque chose monsieur...", type: "thought", action: "scan_crowd" }, 
    { time: 79, text: "Hum, non, je dois confondre avec une star. 😎", type: "speech" },
    
    { time: 87, text: "Excusez-moi, je reçois un appel...", type: "speech", action: "phone_call" }, 
    { time: 95, text: "Allô la régie ? Oui, ici Clap-E, je vous écoute.", type: "speech" },
    { time: 103, text: "QUOI ?! C'est confirmé ?!", type: "speech", action: "surprise" },
    { time: 111, text: "Incroyable ! On vient de me nommer Animateur de la soirée ! 🎤", type: "speech", action: "start_subtitles" },
    { time: 119, text: "Bienvenue à : " + config.titre + " !", type: "speech" },
    { time: 127, text: "Ouhlà... Je n'ai pas préparé mes fiches...", type: "thought", action: "stress_pacing" }, 
    { time: 135, text: "Est-ce que ma batterie est assez chargée ? 🔋", type: "thought" },
    { time: 143, text: "Oui Régie ? Il manque un câble ?", type: "speech", action: "listen_intense" }, 
    { time: 151, text: "Mince ! Clap-E doit filer en coulisses !", type: "speech" },
    
    { time: 157, text: "Je reviens tout de suite ! 🏃‍♂️", type: "speech" }, // Parle
    { time: 162, text: "", action: "enter_teleport_right" }, // SORTIE PAR TELEPORTATION (Plus visible)
    
    { time: 177, text: "Me revoilà ! 😅", type: "speech", action: "enter_teleport_left_visible" }, 
    ...
    { time: 201, text: "Installez-vous bien, Clap-E veille sur vous.", type: "speech" }
];
...
const introScript_VoteOff = [
    ...
    { time: 12, text: "Désolé les amis, les votes sont clos ! 🛑", type: "speech", action: "move_right_slow" },
    { time: 20, text: "La régie est en train de compter les points...", type: "speech" },
    { time: 27, text: "Soyez patients, Clap-E surveille tout ça !", type: "speech", action: "move_left_check" }, 
    { time: 35, text: "Je ne dis rien, mais c'était serré... 🤫", type: "speech" }
];
...
const introScript_Photos = [
    ...
    { time: 18, text: "Poussez-vous, je dois voir les photos !", type: "speech", action: "move_far_left" }, 
    { time: 26, text: "Oh ! Regardez celle-ci ! Superbe !", type: "speech", action: "look_bubbles" }, 
    { time: 33, text: "Je crois que je connais cette personne...", type: "speech", action: "move_right_slow" }, 
    { time: 40, text: "Un petit selfie avec Clap-E ?", type: "speech", action: "pose_selfie" }, 
    { time: 47, text: "Bon, je file voir la régie...", type: "speech" }, 
    { time: 54, text: "", action: "exit_left_fast" }, // Sortie en silence
    { time: 61, text: "Me revoilà de l'autre côté ! ✨", type: "speech", action: "enter_teleport_right_visible" }, 
    { time: 68, text: "Allez, tous ensemble pour la photo !", type: "speech", action: "pose_selfie" } 
];
...
function initThreeJS(canvasFloor, canvasBot, bubbleEl) {
    ...
    // --- PARTICULES BOOSTÉES (500) ---
    const particleCount = 500;
    const particleGeo = new THREE.BufferGeometry();
    const particlePositions = new Float32Array(particleCount * 3);
    particleGeo.setAttribute('position', new THREE.BufferAttribute(particlePositions, 3));
    const particleMat = new THREE.PointsMaterial({ color: 0x00ffff, size: 0.3, transparent: true, opacity: 0 }); // Plus gros
    const particles = new THREE.Points(particleGeo, particleMat);
    sceneBot.add(particles);
    
    // LUMIERE FLASH TELEPORT
    const teleportLight = new THREE.PointLight(0x00ffff, 0, 20);
    sceneBot.add(teleportLight);
    ...
    function triggerTeleport(pos) {
        ...
        teleportLight.position.copy(pos); // Lumière au point de TP
        teleportLight.intensity = 5; // Flash lumineux
        
        particleMat.opacity = 1;
        for(let i=0; i<particleCount; i++) {
            particlePositions[i*3] = (Math.random()-0.5)*3; // Plus large dispersion
    ...
    function animate() {
        ...
        if(isExploding && explosionTime > 0) {
            ...
            teleportLight.intensity = explosionTime * 5; // Flash diminue
        ...
        if (robotState === 'intro') {
            ...
            if (step && time >= step.time) {
                ...
                // TELEPORTATIONS VISIBLES (On set la position du robot SUR l'écran)
                ...
                if(step.action === "enter_teleport_left_visible") {
                    robotGroup.position.set(-12, 0, 0); targetPos.set(-12, 0, 0); triggerTeleport(new THREE.Vector3(-12, 0, 0));
                }
            ...
            if(time > nextEvt) {
                ...
                if(r < 0.05) { 
                    ...
                    robotState = 'dancing'; showBubble("C'est ma chanson préférée !", 'speech');
