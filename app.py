...
if est_admin:
    ...
    with st.sidebar:
        ...
        if st.session_state.get("auth"):
            ...
            # Bouton pour activer/désactiver le texte d'attente
            session_active = st.checkbox("🔔 Activer le texte 'En attente des votes'", value=config.get("session_ouverte", False))
...
# --- 4. MUR LIVE (AFFICHAGE TEXTE CORRIGÉ) ---
    ...
    # --- BLOC TITRES (FIXE) ---
    # Ici on intègre le texte d'attente directement SOUS le titre concours vidéo
    attente_html = ""
    if config.get("session_ouverte", False):
        attente_html = """
            <div style='margin-top:15px;'>
                <span style='background:#E2001A; color:white; padding:10px 30px; border-radius:10px; font-size:30px; font-weight:bold; border:3px solid white; animation: blinker 1.5s linear infinite;'>
    ...
    # --- CONTENU (VOTES OU PHOTOS) ---
    if config["mode_affichage"] == "votes":
        st.bar_chart(json.load(open(VOTES_FILE)))
    else:
        # Logique de photos (Admin ou Live)
        ...
        for p in img_list[-12:]:
            with open(p, "rb") as f:
                ...
                photos_html += f'<img src="data:image/png;base64,{b64}" class="photo" style="width:280px; top:{random.randint(30,65)}%; left:{random.randint(5,80)}%; animation-duration:{random.uniform(10,15)}s;">'
        
        html_content = f"""
        <style>.photo {{ position:absolute; border:5px solid white; border-radius:10px; animation:move alternate infinite ease-in-out; }} @keyframes move {{ from {{ transform:rotate(-2deg); }} to {{ transform:translate(30px,30px) rotate(2deg); }} }}</style>
        <div style="width:100%; height:450px; position:relative;">{photos_html}</div>
        """
    ...
    st.markdown(f"""
        <div style='position:fixed; bottom:30px; left:50%; transform:translateX(-50%); background:white; padding:10px; border-radius:15px; border:5px solid #E2001A; text-align:center;'>
            ...
            <p style="margin:5px 0 0 0; font-weight:bold; font-size:20px; color:black;">{nb_p} PARTICIPANTS</p>
        </div>
    """, unsafe_allow_html=True)

    try:
        from streamlit_autorefresh import st_autorefresh
        st_autorefresh(interval=10000, key="refresh_wall")
    except: pass

# --- 5. UTILISATEUR (VOTE) ---
else:
    st.title("🗳️ Vote Transdev")
    # ... (Le code du vote reste identique : Pseudo, Multi-select, Sécurité LocalStorage)
    pass
