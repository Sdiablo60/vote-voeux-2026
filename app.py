...
# --- 1. INITIALISATION DE L'ÉTAT (AVANT CONFIG) ---
params = st.query_params
est_admin = params.get("admin") == "true"
mode_vote = params.get("mode") == "vote"

# Configuration dynamique de la barre latérale
...
if est_admin:
    ...
    # Barre latérale dédiée à l'admin
    with st.sidebar:
        st.header("🔑 Sécurité")
        pwd = st.text_input("Code Admin", type="password")
        real_pass = "ADMIN_VOEUX_2026" # Vous pouvez changer ce code ici
    ...
    if pwd == real_pass:
        ...
        st.warning("Veuillez saisir le code admin dans la barre latérale à gauche.")
...
# --- 5. MODE LIVE (SOCIAL WALL) ---
    ...
    st.markdown(f"""
        <style>
            ...
            .main-container {{ position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #050505; z-index: 9999; overflow: hidden; }}
            ...
            .welcome-title {{ position: absolute; top: 7%; width: 100%; text-align: center; color: {config['couleur']}; font-family: sans-serif; font-size: {config['taille']}px; font-weight: bold; text-shadow: 0 0 20px {config['couleur']}99; z-index: 10001; animation: pulse-text 4s ease-in-out infinite; }}
            @keyframes pulse-text {{ 0% {{ transform: scale(1); opacity: 0.8; }} 50% {{ transform: scale(1.02); opacity: 1; }} 100% {{ transform: scale(1); opacity: 0.8; }} }}
            .center-point {{ position: absolute; top: 58%; left: 50%; transform: translate(-50%, -50%); width: 180px; height: 180px; z-index: 10000; }}
            ...
            .qr-container {{ position: fixed; bottom: 30px; right: 30px; background: white; padding: 10px; border-radius: 15px; text-align: center; z-index: 10005; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }}
        </style>
        <div class="main-container">
            ...
            <div class="qr-container">
                ...
                <b style="color:black; font-family:sans-serif; font-size:12px;">SCANNEZ POUR VOTER</b>
            </div>
        </div>
