...
if PDF_AVAILABLE:
    class PDFReport(FPDF):
        def header(self):
            ...
            self.cell(0, 5, f"Date du rapport : {datetime.now().strftime('%d/%m/%Y à %H:%M')}", 0, 1, 'R'); self.ln(10)
        def footer(self):
    ...
    def create_pdf_results(title, df, logo_b64=None, total_voters=0):
        ...
        margin_left = 35
        pdf.set_fill_color(226, 0, 26); pdf.set_text_color(255, 255, 255); pdf.set_font("Arial", 'B', 12); pdf.set_x(margin_left)
        ...
        for i, row in df.iterrows():
            ...
            pdf.cell(100, 10, cand, 1, 0, 'C'); pdf.cell(40, 10, str(row['Points']), 1, 1, 'C'); pdf.ln()
    ...
    def create_pdf_audit(title, df, logo_b64=None):
        ...
        pdf.cell(200, 10, txt=f"Audit Detaillé : {title}", ln=True, align='L'); pdf.ln(5)
        cols = [c for c in df.columns.tolist() if "Date" not in c]
        w = 190 / len(cols); pdf.set_fill_color(50, 50, 50); pdf.set_text_color(255)
        for col in cols:
            ...
            if not st.session_state.rules_accepted:
             ...
             if len(choix) == 3:
                if st.button("VALIDER (MODE TEST)", type="primary"):
                    ...
                    for v, p in zip(choix, pts): vts[v] = vts.get(v, 0) + p
                    save_json(VOTES_FILE, vts)
                    st.balloons(); st.success("Vote Test OK"); st.button("🔄 Voter à nouveau (RAZ)", on_click=reset_vote_callback, type="primary"); st.stop()
...
# =========================================================
    ...
    <style>
        ...
        .block-container { padding: 0 !important; margin: 0 !important; max-width: 100% !important; margin-top: -80px !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    if mode == "attente":
        ...
        <html><body style="background:black;margin:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;font-family:Arial;overflow:hidden;">
            ...
            <h1 style="color:white;font-size:100px;font-weight:bold;margin:0;">BIENVENUE</h1>
        ...
        <html><body style="background:black;margin:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;font-family:Arial;overflow:hidden;">
            ...
            <div style="color:white;font-size:30px;margin-top:10px;">{titre_text}</div>
        ...
        <html><head><style>
            ...
            <div style="position:absolute;top:20px;color:white;font-size:30px;font-weight:bold;">{titre_text}</div>
