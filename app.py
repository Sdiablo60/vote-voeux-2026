...
# =========================================================
# GESTION DES IMPORTS OPTIONNELS (PDF)
# =========================================================
try:
...
# =========================================================
# CSS GLOBAL & FONDS D'ECRAN
# =========================================================
st.markdown("""
<style>
    /* 1. FOND GLOBAL BLANC PAR DÉFAUT (Pour l'Admin et le Login) */
    .stApp {
        background-color: #FFFFFF !important;
        color: black;
    }
    
    /* 2. GESTION DU SCROLL (Empêcher les sauts d'image sur TV) */
    ...
    /* Cacher les ascenseurs (Scrollbars) */
    ::-webkit-scrollbar { 
        display: none; 
    }
    ...
    /* 6. STYLE DES BOUTONS D'EXPORT (BLEUS) */
    ...
    /* Le header social est caché par défaut (sauf sur le mur) */
...
# =========================================================
# FONCTIONS UTILITAIRES (JSON, FICHIERS)
# =========================================================
...
def inject_visual_effect(effect_name, intensity, speed):
    if effect_name == "Aucun":
        ...
        return
    # Note: L'injection d'effets (ballons, neige) est désactivée ici pour le mur 
    # car elle est gérée directement dans le JS du mur pour plus de fluidité
    pass 
    ...
    if not st.session_state["auth"]:
        ...
        if "session_active" not in st.session_state or not st.session_state["session_active"]:
            ...
            if menu == "🔴 PILOTAGE LIVE":
                ...
                c1.button("🏠 ACCUEIL", on_click=set_state, args=("attente", False, False), type="primary" if cfg["mode_affichage"]=="attente" else "secondary")
                c2.button("🗳️ VOTES ON", on_click=set_state, args=("votes", True, False), type="primary" if (cfg["mode_affichage"]=="votes" and cfg["session_ouverte"]) else "secondary")
                c3.button("🔒 VOTES OFF", on_click=set_state, args=("votes", False, False), type="primary" if (cfg["mode_affichage"]=="votes" and not cfg["session_ouverte"] and not cfg["reveal_resultats"]) else "secondary")
                c4.button("🏆 PODIUM", on_click=set_state, args=("votes", False, True), type="primary" if cfg["reveal_resultats"] else "secondary")
                st.markdown("---")
                st.button("📸 MUR PHOTOS LIVE", on_click=set_state, args=("photos_live", False, False), type="primary" if cfg["mode_affichage"]=="photos_live" else "secondary")
                st.divider()
                ...
                with t2:
                    ...
                    new_c = c_add.text_input("Nouveau participant")
                    if c_btn.button("➕ Ajouter") and new_c: cfg['candidats'].append(new_c); save_config(); st.rerun()
                    for i, c in enumerate(cfg['candidats']):
                        c1, c2, c3 = st.columns([0.5, 3, 2])
                        with c1:
                            if c in cfg.get("candidats_images", {}): st.image(BytesIO(base64.b64decode(cfg["candidats_images"][c])), width=40)
                        with c2: st.write(c)
                        with c3:
                ...
                st.image([f for f in files[:20]], width=100) 
                ...
                data = [{"Candidat": c, "Points": votes.get(c, 0), "Nb Votes": vote_counts.get(c, 0)} for c in cfg["candidats"]]
    ...
    if "user_pseudo" not in st.session_state:
        ...
        if cfg["mode_affichage"] == "photos_live":
            ...
            if not st.session_state.get("vote_success"):
                ...
                if len(choix) == 3 and st.button("VALIDER (DÉFINITIF)", type="primary"):
...
# =========================================================
    ...
    <style>
        ...
        .social-title {{ color: white; font-size: 40px; font-weight: bold; text-transform: uppercase; }}
    </style>
    ...
    # ------------------ MODE VOTE OUVERT (AVEC COMPTEUR + TAGS) ------------------
        ...
        # FIX: Handle empty participants list
        if nb_total == 0:
