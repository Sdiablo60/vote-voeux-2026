...
# =========================================================
# 3. MUR SOCIAL (VERSION FINALE - FIX INSET)
# =========================================================
else:
    ...
    # === CSS PUISSANT AVEC "INSET" (Fixe définitivement le positionnement) ===
    st.markdown("""
    <style>
        ...
        /* 2. LE BANDEAU TITRE (Fixé en haut - 8vh) */
        .social-header { 
        ...
        /* 3. LA FENÊTRE DU ROBOT (ANCRAGE TOTAL VIA INSET) */
        /* INSET force les 4 cotés. 8vh du haut, 0 du reste. */
        iframe {
            position: fixed !important;
            inset: 8vh 0 0 0 !important; /* HAUT | DROITE | BAS | GAUCHE */
    ...
    # CSS INTERNE IFRAME (Le cadre rouge)
    # On force html et body à 100% pour qu'ils suivent l'iframe qui est maintenant étirée par 'inset'
    ...
    if mode == "attente":
        ...
        html_code = f"""<!DOCTYPE html><html style="height:100%;"><head><style>html, body {{ height: 100%; }} body {{ {style_body_robot} }}{css_content}
            #welcome-text {{ position: absolute; top: 45%; left: 50%; transform: translate(-50%, -50%); text-align: center; color: white; font-family: Arial, sans-serif; z-index: 5; font-size: 70px; font-weight: 900; letter-spacing: 5px; pointer-events: none; transition: all 1s ease-in-out; }}
        </style></head><body>{js_config}<div id="welcome-text">{logo_img_tag}<br>BIENVENUE</div><div id="robot-container" style="z-index: 10; pointer-events: none;"></div>{import_map}<script type="module">{js_content}</script></body></html>"""
        ...
        html_code = f"""<!DOCTYPE html><html style="height:100%;"><head><style>html, body {{ height: 100%; }} body {{ {style_body_robot} }}{css_content}</style></head><body>{js_config}<div id="robot-bubble" class="bubble" style="z-index: 20;">...</div><div id="robot-container" style="z-index: 10; pointer-events: none;"></div>{import_map}<script type="module">{js_content}</script>{bubbles_script}</body></html>"""
        components.html(html_code, height=1000, scrolling=False)
