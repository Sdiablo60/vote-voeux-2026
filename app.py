...
VOTES_FILE, CONFIG_FILE = "votes.json", "config_mur.json"
...
# =========================================================
if est_admin:
    st.markdown("""<style>.header{position:fixed;top:0;left:0;width:100%;height:60px;background:#111;border-bottom:3px solid #E2001A;z-index:999;display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;}.main .block-container{padding-top:80px;}</style><div class="header">ADMINISTRATION</div>""", unsafe_allow_html=True)
    ...
    if not st.session_state["auth"]:
        ...
        st.divider()
        if st.button("📸 MUR PHOTOS", type="primary"): cfg.update({"mode_affichage": "photos_live"}); save_json(CONFIG_FILE, cfg); st.rerun()
        
        st.subheader("Médiathèque")
...
# =========================================================
# 2. APPLICATION MOBILE (SÉCURITÉ MAXIMALE)
# =========================================================
elif est_utilisateur:
    st.markdown("<style>.stApp {background-color:black; color:white;} [data-testid='stHeader'] {display:none;}</style>", unsafe_allow_html=True)
    
    # 1. Vérification Immédiate du "Cookie" de vote
    components.html("""<script>
        if(localStorage.getItem('VOTE_DONE_FINAL_X')) {
    ...
    # 2. Page de Confirmation (C'est ici que l'animation se lance)
    if is_blocked:
        st.balloons() # L'animation se lance MAINTENANT car la page est stable
        st.markdown("""
            <div style='text-align:center; margin-top:100px;'>
                <h1 style='color:#E2001A; font-size:50px;'>MERCI !</h1>
                <h2 style='color:white;'>VOTE ENREGISTRÉ</h2>
                <br><br>
                <p style='color:#888;'>Vous ne pouvez voter qu'une seule fois.</p>
            </div>
    ...
    if "pseudo" not in st.session_state:
        ...
        if cfg["mode_affichage"] == "photos_live":
            st.subheader("📸 Ta photo sur le mur")
            ...
            if len(choix) == 3:
                ...
                if st.button("🚀 VALIDER (DÉFINITIF)", type="primary", use_container_width=True):
                    ...
                    # Marquage Indélébile et Redirection
                    components.html("""<script>
                        localStorage.setItem('VOTE_DONE_FINAL_X', 'true');
                        window.parent.location.href = window.parent.location.href + '&blocked=true';
                    </script>""", height=0)
        else:
            st.info("⏳ En attente...")
...
# =========================================================
# 3. MUR SOCIAL (PODIUM PETIT & NETTOYAGE FORCÉ)
# =========================================================
else:
    ...
    <style>
        ...
        /* GAGNANT : POSITION ABSOLUE PIXEL (400px du haut) */
        .winner-card-final {{ 
            position: fixed; 
            top: 400px; /* Force la position sous le titre */
            left: 50%; transform: translateX(-50%); 
            width: 300px; /* Très petit */
            background: rgba(20,20,20,0.95); border: 5px solid #FFD700; border-radius: 30px; 
            padding: 20px; text-align: center; z-index: 1000; box-shadow: 0 0 40px #FFD700;
        }}
    </style>
    ...
    # --- NETTOYEUR DE CONFETTIS ---
    if mode == "attente":
        # Supprime tout canvas qui traîne
        components.html("""<script>
            const canvasList = window.parent.document.querySelectorAll('canvas');
        ...
        if cfg.get("reveal_resultats"):
            ...
            if sorted_v:
                ...
                <div class="winner-card-final">
                    ...
                    <h1 style="color:white; font-size:30px; margin:10px 0; text-transform: uppercase;">{winner}</h1>
                    <h2 style="color:#FFD700; font-size:18px; margin:0;">VAINQUEUR</h2>
                </div>
                """, unsafe_allow_html=True)
        else:
            ...
            st.markdown(f"""<div style="position:fixed; top:55%; left:50%; transform:translate(-50%, -50%); z-index:1500; background:white; padding:30px; border-radius:30px; text-align:center; border: 10px solid #E2001A;">
                <img src="data:image/png;base64,{qr_b64}" width="230"><h2 style="color:black; margin-top:20px; font-size:22px;">SCANNEZ POUR VOTER</h2></div>""", unsafe_allow_html=True)
