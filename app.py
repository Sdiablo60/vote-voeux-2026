...
# =========================================================
# 3. MUR SOCIAL (VERSION FINALE - DESIGN AJUSTÉ ROUGE)
# =========================================================
else:
    ...
    # === CSS INTERNE IFRAME (DESIGN AJUSTÉ) ===
    internal_css = f"""
    <style>
        body {{ margin: 0; padding: 0; background-color: black; overflow: hidden; width: 100vw; height: 100vh; }}
        #safe-zone {{
            position: absolute; top: 8vh; left: 0; width: 100vw; height: 92vh;
            box-sizing: border-box; z-index: 100; pointer-events: none;
        }}
        {css_content}
        
        /* CONTENEUR CENTRAL REMONTÉ */
        #welcome-container {{
            position: absolute; 
            top: 40%; /* <-- REMONTÉ (était 52%) pour rapprocher du titre */
            left: 50%; transform: translate(-50%, -50%);
            text-align: center; width: 80%; z-index: 5; pointer-events: none;
        }}
        
        /* LOGO AVEC PLUS D'ESPACE DESSOUS */
        #welcome-logo {{
            width: 380px;
            margin-bottom: 60px; /* <-- ESPACE AUGMENTÉ vers Bienvenue */
        }}
        
        /* NÉON ROUGE VIF (Rouge + Rouge Transdev) */
        #welcome-title {{
            font-family: Arial, sans-serif;
            font-size: 70px; font-weight: 900; letter-spacing: 5px;
            margin: 0; padding: 0;
            color: #fff; /* Coeur blanc */
            text-shadow:
                0 0 5px #fff,
                0 0 10px #ffcccc, /* Halo blanc-rouge */
                0 0 20px #ff0000, /* ROUGE VIF */
                0 0 40px #E2001A, /* ROUGE TRANSDEV */
                0 0 80px #E2001A,
                0 0 90px #ff0000,
                0 0 100px #E2001A,
                0 0 150px #ff0000;
        ...
        @keyframes neon-flicker {{
            0%, 19%, 21%, 23%, 25%, 54%, 56%, 100% {{
                text-shadow:
                    0 0 5px #fff, 0 0 10px #ffcccc, 0 0 20px #ff0000, 0 0 40px #E2001A,
                    0 0 80px #E2001A, 0 0 90px #ff0000, 0 0 100px #E2001A, 0 0 150px #ff0000;
            }}
            20%, 24%, 55% {{        
        ...
        /* SOUS-TEXTE AVEC PLUS D'ESPACE DESSUS */
        #sub-text {{
            margin-top: 50px; /* <-- ESPACE AUGMENTÉ sous Bienvenue */
            color: #eeeeee;
            font-family: 'Arial', sans-serif;
            font-size: 40px;
            font-weight: normal;
    ...
    if mode == "attente":
        ...
        html_code = f"""<!DOCTYPE html><html><head>{internal_css}</head><body>{js_config}
            ...
            <div id="welcome-container">
                {logo_img_tag}
                <div id="welcome-title">BIENVENUE</div>
                <div id="sub-text"></div>
            </div>
