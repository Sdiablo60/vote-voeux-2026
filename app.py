...
# --- 0. PRIORITÉ ABSOLUE : FORCER LE NOIR POUR LE MUR ---
st.markdown("""
    <style>
...
if est_admin:
    # On force un style clair spécifique pour l'admin pour voir ce qu'on fait
    st.markdown("""
        <style>
        html, body, .stApp { 
        ...
        .stMarkdown, p, h1, h2, h3, label { color: #31333F !important; } /* Texte sombre */
        </style>
    """, unsafe_allow_html=True)
    
    st.title("⚙️ Console d'Administration")
    
    input_pwd = st.text_input("Entrez le code secret pour déverrouiller", type="password")
    ...
    if input_pwd == "ADMIN_LIVE_MASTER":
        ...
        with col1:
            ...
            ul = st.file_uploader("Charger un nouveau logo (PNG/JPG)", type=['png', 'jpg', 'jpeg'])
            if ul:
                with open(LOGO_FILE, "wb") as f: f.write(ul.getbuffer())
                st.info("Logo mis à jour !")
        
        with col2:
            st.subheader("Ajouter des Photos")
            up = st.file_uploader("Sélectionnez vos images", accept_multiple_files=True)
            if up:
                for f in up:
                    with open(os.path.join(GALLERY_DIR, f.name), "wb") as file: file.write(f.getbuffer())
                st.success(f"{len(up)} photos ajoutées !")
        ...
        st.warning("Veuillez saisir le code pour accéder aux réglages.")
        st.stop()

# --- 3. STYLE CSS DU MUR (S'applique uniquement si pas admin) ---
else:
    st.markdown("""
        <style>
...
if not est_admin:
    if not mode_vote:
        ...
        # MODE VOTE (Couleurs sombres mais lisibles)
