...
# =========================================================
# 3. MUR SOCIAL (FOND NOIR, COMPONENTS, JS CORRIGÉ)
# =========================================================
else:
    ...
    # --- CSS AGRESSIF POUR LE MUR SOCIAL (HIDE HEADER) ---
    st.markdown("""
    <style>
    ...
    # --- 3. VOTE ON (COMPONENTS + TICKER) ---
        ...
        <style>
            ...
            .user-tag {{ 
                ...
                animation: popIn 0.5s ease-out; display: inline-block;
            }}
    ...
    # --- 5. PHOTOS LIVE (COMPONENTS + JS FIXED) ---
    elif mode == "photos_live":
        photos = glob.glob(f"{LIVE_DIR}/*")
        js_imgs = json.dumps([f"data:image/jpeg;base64,{base64.b64encode(open(f, 'rb').read()).decode()}" for f in photos[-40:]])
        
        # Utilisation de CONCATÉNATION JS classique (+) au lieu de f-string pour éviter le bug Python/JS
        html = f"""
        ...
        <script>
            ...
            imgs.forEach(src => {{
                ...
                img.style.cssText = 'position:absolute;width:'+(Math.random()*100+80)+'px;border-radius:50%;border:3px solid #E2001A;object-fit:cover;';
