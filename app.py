    ...
    if mode == "attente":
        ...
        if cfg.get("reveal_resultats"):
            ...
            def get_row_html(cands, score, emoji, rank_class):
                if not cands: return ""
                count = len(cands)
                # Logique de redimensionnement si beaucoup de gagnants dans une ligne
                scale = 1.0
                if count >= 2: scale = 0.8  # Reduce size if 2 or more
                if count >= 3: scale = 0.7  # Further reduce if 3 or more
                ...
                font_n = int(24 * scale)
                font_s = int(18 * scale)
                margin_bottom = 10 # Default margin
                
                # Special sizing for 1st place
                if "rank-1" in rank_class: 
                    card_w = int(350 * scale)
                    img_s = int(160 * scale)
                    font_n = int(36 * scale)
                
                # Reduce margin if multiple cards stacked (though they are flex row here, this logic is kept if flex-direction changes or margins affect layout)
                if count > 1:
                     margin_bottom = 5
                ...
                for c in cands:
                    ...
                    html += f"""
                    <div class='{rank_class}' style='margin:0 5px;'>
                        <div class='p-card' style='width:{card_w}px; margin-bottom:{margin_bottom}px;'>
                            {img_tag}
            ...
            # ... (rest of the 'session_ouverte' block remains unchanged)
