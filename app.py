...
if PDF_AVAILABLE:
    ...
    def create_pdf_results(title, df, logo_data=None):
        # Sauvegarde temporaire du logo pour le PDF
        if logo_data:
            try:
        ...
        # Marges pour centrer (A4 = 210mm)
        # Col1=100, Col2=40 => Total 140. Marge = (210-140)/2 = 35
        pdf.set_x(35)
    ...
    def create_pdf_audit(title, df, logo_data=None):
        ...
        pdf.set_font("Arial", 'B', 14)
        pdf.cell(200, 10, txt=f"Audit Détail : {title}", ln=True, align='L')
        pdf.ln(5)
            ...
            if menu == "🔴 PILOTAGE LIVE":
                ...
                with c_top2:
                    if files:
                        ...
                        st.download_button("⬇️ TOUT TÉLÉCHARGER (ZIP)", data=zip_all.getvalue(), file_name="toutes_photos_live.zip", mime="application/zip", use_container_width=True)
...
# =========================================================
# 2. FONCTIONS D'AFFICHAGE ISOLÉES (SILO)
# =========================================================
...
# ---------------------------------------------------------
# 3. MUR SOCIAL : DISPATCHER
# ---------------------------------------------------------
def afficher_mur_social():
    ...
    if mode == "attente":
        afficher_mur_attente(cfg)
    elif mode == "votes":
        if cfg.get("reveal_resultats"):
            afficher_mur_podium(cfg)
        elif cfg.get("session_ouverte"):
            afficher_mur_vote_on(cfg)
        else:
            afficher_mur_vote_off(cfg)
    elif mode == "photos_live":
        afficher_mur_photos_live(cfg)

def afficher_mur_attente(cfg):
    ph = st.empty()
...
def afficher_mur_photos_live(cfg):
    host = st.context.headers.get('host', 'localhost')
    qr_buf = BytesIO(); qrcode.make(f"https://{host}/?mode=vote").save(qr_buf, format="PNG")
    qr_b64 = base64.b64encode(qr_buf.getvalue()).decode()
    logo_data = cfg.get("logo_b64", "")
