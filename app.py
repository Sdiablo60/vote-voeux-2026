...
# --- CSS GLOBAL (INTERFACE ADMIN & LOGIN) ---
st.markdown("""
<style>
    ...
    .custom-link-btn:hover { transform: scale(1.02); text-decoration: none; }
...
# =========================================================
if est_admin:
    # Force le fond clair uniquement pour l'admin
...
# =========================================================
    ...
    st.markdown("""<style>.stApp {background-color:black !important; color:white !important;} [data-testid='stHeader'] {display:none;}</style>""", unsafe_allow_html=True)
...
# =========================================================
# 3. MUR SOCIAL (NOIR, FULL SCREEN, JS)
# =========================================================
else:
    ...
    title_text = cfg.get("titre_mur", "")
    
    # --- 1. ACCUEIL ---
    if mode == "attente":
        html = f"""
        <html><body style="background:black;margin:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;overflow:hidden;">
            {f'<img src="data:image/png;base64,{logo_data}" style="width:400px;margin-bottom:30px;">' if logo_data else ''}
            <h1 style="color:white;font-family:Arial;font-size:80px;margin:0;">BIENVENUE</h1>
            <h2 style="color:#E2001A;font-family:Arial;font-size:40px;margin-top:20px;">{title_text}</h2>
        </body></html>
        """
    ...
    # --- 2. VOTE OFF ---
        ...
        <html><body style="background:black;margin:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;overflow:hidden;">
            <div style="border:5px solid #E2001A; padding:50px; border-radius:30px; text-align:center;">
                {f'<img src="data:image/png;base64,{logo_data}" style="width:200px;margin-bottom:20px;">' if logo_data else ''}
                <h1 style="color:#E2001A;font-family:Arial;font-size:60px;margin:0;">VOTES CLÔTURÉS</h1>
                <h2 style="color:white;font-family:Arial;font-size:30px;margin-top:20px;">{title_text}</h2>
            </div>
        </body></html>
        """
        components.html(html, height=900)
    ...
    # --- 3. VOTE ON ---
        ...
        <html><body style="background:black;margin:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;font-family:Arial;overflow:hidden;">
            {f'<img src="data:image/png;base64,{logo_data}" style="width:300px;margin-bottom:30px;">' if logo_data else ''}
            ...
            <div style="color:white;font-size:20px;margin-top:10px;">{title_text}</div>
    ...
    # --- 4. PODIUM ---
        ...
        <html><head><style>
            ...
            .grid {{ display:flex; justify-content:center; align-items:flex-end; gap:30px; }}
            ...
            .win {{ background:rgba(20,20,20,0.9); border:4px solid #FFD700; width:400px; padding:30px; box-shadow:0 0 50px #FFD700; }}
            .win img {{ width:200px; height:200px; border-radius:50%; border:5px solid white; object-fit:cover; margin-bottom:15px; }}
            .win h1 {{ font-size:40px; margin:5px 0; color:white; }}
        </style></head><body>
            ...
            <script>
                ...
                function card(p, rank) {{
                    if(rank==1) return `<div class="win"><div style="font-size:60px">🥇</div>`+(p.i?`<img src="${{p.i}}">`:``)+`<h1>${{p.n}}</h1><h2 style="color:#FFD700">VAINQUEUR</h2><h3 style="color:#CCC">${{p.s}} pts</h3></div>`;
                ...
                setInterval(() => {{
                    ...
                    // PHASE 3: 1er (25-30s intro, 30s+ affiche VAINQUEUR SEUL)
                    else if(el < 30) {{ showIntro=true; txt="LE VAINQUEUR..."; time=30-el; }}
                    else {{
                        intro.style.display='none'; final.style.display='flex';
                        let h=""; r1.forEach(p=>h+=card(p,1)); grid.innerHTML=h; // AFFICHE JUSTE LE 1ER
                    }}
