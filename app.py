...
def archive_current_session(name_suffix="AutoSave"):
    ...
    folder_name = f"{timestamp}_{sanitize_filename(titre)}_{name_suffix}"
...
def inject_visual_effect(effect_name, intensity, speed):
    ...
    js_code = f"""
    <script>
        var doc = window.parent.document; var layer = doc.getElementById('effect-layer');
        if(!layer) {{ layer = doc.createElement('div'); layer.id = 'effect-layer'; layer.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;pointer-events:none;z-index:0;overflow:hidden;'; doc.body.appendChild(layer); }}
        function createBalloon() {{ var e = doc.createElement('div'); e.innerHTML = '🎈'; e.style.cssText = 'position:absolute;bottom:-100px;left:'+Math.random()*100+'vw;font-size:'+(Math.random()*40+30)+'px;transition:bottom {max(3, 25-(speed*0.4))}s linear;'; layer.appendChild(e); setTimeout(() => {{ e.style.bottom = '110vh'; }}, 50); setTimeout(() => {{ e.remove(); }}, {max(3, 25-(speed*0.4))*1000}); }}
        function createSnow() {{ var e = doc.createElement('div'); e.innerHTML = '❄'; e.style.cssText = 'position:absolute;top:-50px;left:'+Math.random()*100+'vw;color:white;font-size:'+(Math.random()*20+10)+'px;transition:top {max(3, 25-(speed*0.4))}s linear;'; layer.appendChild(e); setTimeout(() => {{ e.style.top = '110vh'; }}, 50); setTimeout(() => {{ e.remove(); }}, {max(3, 25-(speed*0.4))*1000}); }}
    """
...
if PDF_AVAILABLE:
    class PDFReport(FPDF):
        def header(self):
            ...
            self.set_xy(x_pos, 10); self.cell(0, 10, 'REGIE MASTER - RAPPORT OFFICIEL', 0, 1, 'C'); self.ln(5)
            self.set_font('Arial', 'I', 10); self.set_text_color(100)
            self.cell(0, 10, f"Généré le {datetime.now().strftime('%d/%m/%Y à %H:%M')}", 0, 1, 'R'); self.ln(10)
        def footer(self):
    ...
    def prepare_logo_file(logo_b64):
        if logo_b64:
            try:
                with open("temp_logo.png", "wb") as f: f.write(base64.b64decode(logo_b64))
                return True
            except: return False
        return False
    ...
    def create_pdf_results(title, df, logo_data=None, total_voters=0):
        try:
            ...
            for i, row in df.iterrows():
                pdf.set_x(35); pdf.cell(100, 10, str(row['Candidat']).encode('latin-1','replace').decode('latin-1'), 1, 0, 'C')
                pdf.cell(40, 10, str(row['Points']), 1, 1, 'C'); pdf.ln()
    ...
    def create_pdf_audit(title, df, logo_data=None):
        try:
            ...
            for col in cols: pdf.cell(w, 8, col.encode('latin-1','replace').decode('latin-1'), 1, 0, 'C', 1)
            pdf.ln(); pdf.set_text_color(0)
            for i, row in df.iterrows():
                for col in cols: pdf.cell(w, 8, str(row[col]).encode('latin-1','replace').decode('latin-1'), 1, 0, 'C')
                pdf.ln()
            if os.path.exists("temp_logo.png"): os.remove("temp_logo.png")
            return pdf.output(dest='S').encode('latin-1')
        except: return b"Erreur PDF"
...
def afficher_console_admin():
    ...
    if not st.session_state["auth"]:
        ...
        if "session_active" not in st.session_state or not st.session_state["session_active"]:
            ...
            with c1:
                st.markdown("### 🚀 Continuer"); current_title = st.session_state.config.get("titre_mur", "Session Inconnue"); st.write(f"Session : **{current_title}**")
                if st.button("OUVRIR LA SESSION", type="primary", use_container_width=True): st.session_state["session_active"] = True; st.rerun()
            with c2:
                st.markdown("### ✨ Créer"); st.write("Nouvelle session vierge")
                if st.button("CRÉER NOUVELLE SESSION", type="primary", use_container_width=True): archive_current_session("AutoSave"); reset_app_data("blank"); st.session_state["session_active"] = True; st.rerun()
            ...
            for arc in archives:
                with st.expander(f"📁 {arc}"):
                    ...
                    if c1.button(f"Restaurer {arc}", key=f"res_{arc}"): archive_current_session("PreRestore"); restore_session_from_archive(arc); st.session_state.config = load_json(CONFIG_FILE, default_config); st.session_state["session_active"] = True; st.rerun()
                    if c2.button("🗑️", key=f"del_{arc}"): delete_archived_session(arc); st.rerun()
        else:
            ...
            with st.sidebar:
                ...
                st.divider(); st.markdown("""<a href="/" target="_blank" class="custom-link-btn btn-red">📺 OUVRIR MUR SOCIAL</a><a href="/?mode=vote&test_admin=true" target="_blank" class="custom-link-btn btn-blue">📱 TESTER MOBILE (ILLIMITÉ)</a>""", unsafe_allow_html=True)
            ...
            if st.session_state.admin_menu == "🔴 PILOTAGE LIVE":
                ...
                with t2:
                    ...
                    new_cand = c_add.text_input("Nouveau participant")
                    if c_btn.button("➕ Ajouter") and new_cand: cfg['candidats'].append(new_cand.strip()); save_config(); st.rerun()
                    st.divider()
                    for i, cand in enumerate(cfg['candidats']):
                        c1, c2, c3 = st.columns([0.5, 3, 2])
                        if cand in cfg.get("candidats_images", {}): c1.image(BytesIO(base64.b64decode(cfg["candidats_images"][cand])), width=40)
                        c2.text_input(f"P{i+1}", value=cand, disabled=True)
                        u = c3.file_uploader(f"Img {i}", type=["png","jpg"], key=f"up_{i}", label_visibility="collapsed")
                        if u: st.session_state.config["candidats_images"][cand] = process_participant_image(u); save_config(); st.rerun()
...
# =========================================================
# 2. APPLICATION MOBILE (SILO)
# =========================================================
elif est_utilisateur:
    def afficher_mobile_vote():
        ...
        if "user_pseudo" not in st.session_state:
            ...
            if st.button("ENTRER", type="primary", use_container_width=True) and pseudo: st.session_state.user_pseudo = pseudo.strip(); parts = load_json(PARTICIPANTS_FILE, []); parts.append(pseudo.strip()); save_json(PARTICIPANTS_FILE, parts); st.rerun()
        else:
            if cfg["mode_affichage"] == "photos_live":
                ...
                uploaded_file = st.file_uploader("Choisir dans la galerie", type=['png', 'jpg', 'jpeg'], key=up_key); cam_file = st.camera_input("Prendre une photo", key=cam_key); final_file = uploaded_file if uploaded_file else cam_file
                if final_file:
                ...
                if not st.session_state.rules_accepted:
                    ...
                    if len(choix) == 3:
                        if st.button("VALIDER (DÉFINITIF)", type="primary", use_container_width=True):
                            ...
                            save_json(VOTES_FILE, vts); details = load_json(DETAILED_VOTES_FILE, []); details.append({"Utilisateur": st.session_state.user_pseudo, "Choix 1 (5pts)": choix[0], "Choix 2 (3pts)": choix[1], "Choix 3 (1pt)": choix[2], "Date": datetime.now().strftime("%H:%M:%S")}); save_json(DETAILED_VOTES_FILE, details)
    ...
    afficher_mobile_vote()

# =========================================================
# 3. MUR SOCIAL (SILO)
# =========================================================
else:
    def afficher_mur_attente(cfg, logo_data, titre_text):
        html = f"""
    ...
    def afficher_mur_vote_off(cfg, logo_data, titre_text):
        html = f"""
        <html><body style="background:black;margin:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;font-family:Arial;overflow:hidden;">
            <div style="position:fixed;top:30px;width:100%;text-align:center;z-index:1000;"><h1 style="color:#E2001A;font-family:Arial;font-weight:bold;font-size:50px;text-transform:uppercase;">{titre_text}</h1></div>
    ...
    def afficher_mur_vote_on(cfg, logo_data, titre_text):
    ...
    def afficher_mur_podium(cfg, logo_data, titre_text):
    ...
    def afficher_mur_photo_live(cfg, logo_data, titre_text):
    ...
    if mode == "attente":
        afficher_mur_attente(cfg, logo_data, titre_text)
    elif mode == "votes":
        if cfg.get("reveal_resultats"):
            afficher_mur_podium(cfg, logo_data, titre_text)
        elif cfg.get("session_ouverte"):
            afficher_mur_vote_on(cfg, logo_data, titre_text)
        else:
            afficher_mur_vote_off(cfg, logo_data, titre_text)
    elif mode == "photos_live":
        afficher_mur_photo_live(cfg, logo_data, titre_text)
