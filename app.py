...
# =========================================================
    ...
    # === CSS SPÉCIFIQUE AU MUR SOCIAL (Injecté seulement ici) ===
    st.markdown("""
    <style>
        /* 1. Fond NOIR et suppression des marges (Force Edge-to-Edge) */
    ...
    # CSS INTERNE IFRAME (Ligne rouge)
    ...
    if mode == "attente":
        ...
        html_code = f"""<!DOCTYPE html><html><head><style>body {{ {style_body_robot} }}{css_content}
        ...
        if cfg.get("reveal_resultats"):
            # ... (Code Podium identique, je l'abrège pour la lisibilité, copiez le bloc podium précédent si besoin) ...
            # Pour faire simple, reprenez le bloc podium de votre version précédente ou celle que j'ai donnée avant.
            # L'important ici est le CSS injecté au début du 'else'.
            # Je remets le bloc podium complet ci-dessous pour être sûr :
        ...
        # ... (Photos Live identique, je l'abrège) ...
        # Copiez le bloc "photos_live" du code précédent.
        host = st.context.headers.get('host', 'localhost')
        qr_buf = BytesIO(); qrcode.make(f"https://{host}/?mode=vote").save(qr_buf, format="PNG")
        qr_b64 = base64.b64encode(qr_buf.getvalue()).decode()
        logo_data = cfg.get("logo_b64", "")
