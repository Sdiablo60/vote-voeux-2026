...
# --- 4. MUR SOCIAL (DISPOSITION CORRIGÉE) ---
    ...
    # BLOC HAUT : TITRE + COMPTEUR + DOUBLE QR
    attente_html = ""
    ...
    # Affichage dynamique (Sécurité : si v_data est vide, on force les photos)
    if config["mode_affichage"] == "votes" and len(v_data) > 0 and any(v > 0 for v in v_data.values()):
        st.bar_chart(v_data)
    else:
    ...
    try:
        ...
        st_autorefresh(interval=5000, key="force_wall_update_v99")
    except: pass
    ...
    if st.session_state.get("voted"):
        ...
        if "user_pseudo" not in st.session_state:
            ...
            if not config.get("session_ouverte", False):
                try:
                    from streamlit_autorefresh import st_autorefresh
                    st_autorefresh(interval=5000, key="mobile_check_v99")
                except: pass
                st.markdown(f"<div style='text-align:center; padding:40px; border:2px dashed #E2001A; border-radius:15px; margin-top:20px;'><h2 style='color:#E2001A;'>⌛ En attente ouverture des votes...</h2><p>Bienvenue {st.session_state['user_pseudo']}. Le formulaire apparaîtra tout seul.</p></div>", unsafe_allow_html=True)
            else:
